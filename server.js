...
const { gerarPrompt } = require('./prompt.js');

// Google Docs prompt cache
const GDOC_ID = '1qq4UyWizTE7P_IEX-WUIlDC_ETwlZu2ZwB5_B7UwiX0';
let promptCache = null;
let promptCacheTime = 0;
const PROMPT_CACHE_TTL = 5 * 60 * 1000; // 5 minutos

async function buscarPromptDoDrive() {
  const agora = Date.now();
  if (promptCache && (agora - promptCacheTime) < PROMPT_CACHE_TTL) {
    return promptCache;
  }
  try {
    const url = `https://docs.google.com/document/d/${GDOC_ID}/export?format=txt`;
    const res = await fetch(url);
    if (!res.ok) throw new Error('Falha ao buscar prompt do Drive');
    const texto = await res.text();
    promptCache = texto.trim();
    promptCacheTime = agora;
    console.log('[PROMPT] Atualizado do Google Drive');
    return promptCache;
  } catch(e) {
    console.warn('[PROMPT] Erro ao buscar do Drive, usando prompt padrão:', e.message);
    return null;
  }
}
const fs = require('fs');
  ...
  if (req.method === 'POST' && req.url === '/api/gerar') {
    ...
    req.on('end', async () => {
      try {
        ...
        // Compute variables for Drive prompt substitution
        const tipoMap = {
        ...
        const finishMap = {
          'Bronze Envelhecido': 'antique bronze plating: warm golden-dark tone with deep grey-black patina filling all recesses, matte-satin surface',
          'Prata Envelhecido': 'antique silver plating: cold grey-silver tone with near-black patina deep in all recesses, matte-frosted surface',
          'Ouro Brilho': 'polished gold plating: intense saturated warm yellow-gold, highly reflective mirror-polished surface',
          'Cobre Envelhecido': 'antique copper plating: warm reddish-pink copper tone with dramatic near-black patina in all recesses',
          'Níquel Brilho': 'polished nickel plating: bright cold white-silver tone, highly reflective mirror-polished surface',
          'Prata Brilho': 'polished silver plating: bright cold white-silver tone, highly reflective mirror-polished surface'
        };
        const finish = finishMap[material] || finishMap['Bronze Envelhecido'];

        const enamelDesc = hasColor
          ? 'vibrant hard enamel fills in the recessed areas with flat saturated colors, opaque and glossy'
          : 'no enamel or color fills — pure metallic surface throughout';

        const shapeDesc = isBrève
          ? 'custom shield or badge shape, thin profile designed to be pinned to a uniform'
          : 'perfectly circular coin shape with defined edge rim, thick profile';

        const compositionDesc = isBrève
          ? `single ${tip} centered in frame: ${promptLado1 || 'decorative front face'}`
          : `two ${tip}s side by side: LEFT (front): ${promptLado1 || 'decorative front face'}. RIGHT (back): ${promptLado2 || 'decorative back face'}`;

        // Tenta buscar prompt customizado do Google Drive
        const promptDrive = await buscarPromptDoDrive();
        let prompt;
        if (promptDrive && promptDrive.length > 50) {
          // Substitui variáveis no prompt do Drive
          prompt = promptDrive
            .replace(/\{\{tipo\}\}/g, tip)
            .replace(/\{\{material\}\}/g, finish)
            .replace(/\{\{esmalte\}\}/g, enamelDesc)
            .replace(/\{\{formato\}\}/g, shapeDesc)
            .replace(/\{\{composicao\}\}/g, compositionDesc)
            .replace(/\{\{lado1\}\}/g, promptLado1 || 'decorative front face')
            .replace(/\{\{lado2\}\}/g, promptLado2 || 'decorative back face');
        } else {
          prompt = gerarPrompt({ tipo, material, cores, promptLado1, promptLado2 });
        }
