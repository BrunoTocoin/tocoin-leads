  ...
  if (req.method === 'POST' && req.url === '/api/gerar') {
    ...
    req.on('end', async () => {
      try {
        ...
        const prompt = `Photorealistic macro product photography of a Brazilian custom ${tip} manufactured by Tocoin Moedas e Medalhas. ${compositionDesc}. PLATING: ${finish}. ENAMEL: ${enamelDesc}. SHAPE: ${shapeDesc}. PHOTOGRAPHY STYLE: coin is positioned PERFECTLY FLAT and HORIZONTAL, camera angle is 100% perpendicular to the coin face (0 degrees tilt, 0 degrees rotation, 0 degrees perspective), the coin must appear as a perfect circle with no ellipse or distortion, as if photographed from directly above in a completely flat overhead position — like a scanner. Dramatic side lighting from the left creating relief depth without tilting the coin. Dark charcoal background (#3d3d3c). Ultra-sharp macro focus, 8k resolution, professional numismatic product photography. The coin must NOT be leaning, tilted, angled or rotated in any direction.`;
